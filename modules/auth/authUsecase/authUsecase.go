...
func (u *authUsecase) LogIn(pctx context.Context, cfg *config.Config, req *auth.PlayerLoginReq) (*auth.ProfileIntercepter, error) {
	profile, err := u.authRepository.CredentailSearch(pctx, cfg.Grpc.PlayerUrl, &playerPb.CredentialSearchReq{
		Email:    req.Email,
	...
	// 	PlayerId: profile.Id,
	// 	RoleCode: int(profile.RoleCode),
	// }).SignToken(&jwt.SigningMethodRSA{})
	...
	log.Printf("Player Profile: %s", profile.String())
	log.Printf("profile.Id: %s", profile.Id)
	log.Printf("profile.RoleCode: %d", int(profile.RoleCode))
	log.Printf("cfg.Jwt.AccessDuration: %d", cfg.Jwt.AccessDuration)
	log.Printf("cfg.Jwt.RefreshDuration: %d", cfg.Jwt.RefreshDuration)
